...
Node* createNode(const char* name, NodeType type, Node* parent) {
    ...
    if (type == DIRECTORY) {
        ...
        // inode 초기화
        newNode->file.inode.fileSize = sizeof(newNode->file.content);
...
void searchfile(Node* node, const char* keyword) {
    if (node->type == FILE_TYPE) {
        if (strstr(node->file.content, keyword) != NULL) { // 파일 내용에 키워드가 포함되어 있는지 확인
            ...
            printf("파일 크기: %d바이트, 생성 시간: %ld, 수정 시간: %ld\n", node->file.inode.fileSize, (long)node->file.inode.created, (long)node->file.inode.modified);
        }
    } else if (node->type == DIRECTORY) {
        for (int i = 0; i < node->dir.childCount; i++) {
...
int main() {
    ...
    while (1) {
        printf("명령을 입력하세요 (makedir, makefile, readfile, updatefile, searchfile, print, delete, rename, copy, quit): ");
        scanf("%s", command);
